...
func (db *EndGameDb) retrogradeAnalysisStepN(dtm int) (noError error) {
	...
	if player == WHITE {
		for _, a := range db.AnalysisMap {
			...
			for _, m := range moves {
				newBoard := a.board.DoMove(m)
				db.addDTMToAnalysis(newBoard, positionDtm+1, m)
				newMovesFound++
			}
		}
	} else {
		for _, a := range db.AnalysisMap {
			...
			if found == len(moves) {
				for _, m := range moves {
					db.addDTMToAnalysis(a.board, minDTM+1, m)
					newMovesFound++
				}
			}
		}
...
func (db *EndGameDb) retrogradeAnalysisStepNforWhite(dtm int) (noError error) {
	...
	for _, a := range db.AnalysisMap {
		...
		for _, d := range a.dtmWhite {
			db.addDTMToAnalysis(d.board, positionDtm+1, d.move)
			newMovesFound++
		}
	}
	end := time.Now()
...
func (db *EndGameDb) retrogradeAnalysis() {
	...
	db.retrogradeAnalysisStepNForBlack(2)
